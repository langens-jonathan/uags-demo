...
public class SplitQuery implements Iterable<String>
{
    ...
    private void performSplitQuery(String query)
    {
        ...
        boolean inExistsFilter = false;

        for(byte b : query.getBytes())
        {
            if(!inString && !inComments &&!inURI && !inExistsFilter) {
                if (((char) b) == ' ' || ((char) b) == '\t') {
                    currentBuffer = currentBuffer.trim();
                    if (currentBuffer.length() > 0)
                    {
                        splitQuery.add(currentBuffer);
                        if(currentBuffer.toLowerCase().startsWith("filter"))
                            inFilter = true;
                        if(inFilter && currentBuffer.toLowerCase().startsWith("exists"))
                            inExistsFilter = true;
                    }
                    currentBuffer = "";
                    continue;
                ...
                {
                    currentBuffer = currentBuffer.trim();
                    if(!currentBuffer.isEmpty())splitQuery.add(currentBuffer);
                    splitQuery.add(""+((char)b));
                    currentBuffer = "";
                    continue;
                }
